...
void WChL(double** W,double** covM,double** corrM, int* perm, int D)
{
    ChLT chow(corrM,covM,D,2);
    chow.build();
    //Pick a root and turn the graph to a DAG
    Graph<int,int> DAGC(true);
    int root=chow.getBestVertex();
    DAGC.fillDAG(root,chow.structure);
    //The DFS in a tree gives a permutation of nodes, which turns out to be an ancestral ordering of the tree.
    vector<int> permut=DAGC.DepthFirstSearch(root);


    vector<double> beta(D,0.0);
    vector<double> cv(D,0.0);
    //Computing conditional variances, and regression coefs, for order 2 only
...
void getW(double** W, double** covM,double** corrM, double* perm, int D, int k)
{
    ...
    vector<int> p;
    if(k<=0)
        cout<<"Can't build a t-Cherry Junction Tree of order lower than 1";
